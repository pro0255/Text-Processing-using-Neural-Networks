...
class ExperimentLoader:
    ...
    def create_dataset_generator(self, number_of_authors, number_of_sentences, preprocessing_types, norm_sizes):

        load_configs = list(itertools.product(number_of_authors, number_of_sentences, preprocessing_types, norm_sizes))
        ...
        for conf in load_configs:
            ...
            loaded_data = load_dataset_from_path_with_normalization(data_path, None, current_preprocessing)
            normalized_loaded_data = normalize_dataframe_to_size(loaded_data, norm_size)
            X_train, X_valid, X_test, y_train, y_valid, y_test = split_dataframe_to_train_test_valid(normalized_loaded_data)

            yield (X_train, X_valid, X_test, y_train, y_valid, y_test), loaded_data, paths, conf