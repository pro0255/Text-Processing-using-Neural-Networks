...
from src.data_preparing.build.chunk_document_by_sentence import chunk_document_by_sentence
...
def build_process_func(k, name, path, authors_tuple):
    ...
    directory_for_file = os.sep.join(
        [
            path, 
            f'{number_of_authors}{GutenbergLabelType.Authors.value}', name
        ]
    )
    ...
    def process_to_create_dataset(data):
        ...
        try:
            current_gutenberg_id = get_data_from_gutenberg(data, GutenbergJsonAttributes.Id.value)
            current_text = get_data_from_gutenberg(data, GutenbergJsonAttributes.Text.value)
            current_author_id = get_data_from_gutenberg(data, GutenbergJsonAttributes.AuthorId.value)
            ...
            log_error(directory_for_file, get_data_from_gutenberg(data, GutenbergJsonAttributes.Id.value), e)
