...
class TransformerTrainExperiment:
    ...
    def get_model(self, trainable):
        ...
        y = tf.keras.layers.Dense(
            self.number_of_authors, activation="softmax", name="outputs"
        )(X)
    ...
    def run(self):
        ...
        path_data, path_authors = get_path_to_gutenberg_sets(
            self.number_of_authors, self.number_of_sentences, current_folder
        )

        preprocessing_factory = PreprocessingFactory()

        train, valid, test = get_datasets(
            path_data, ";", preprocessing_factory.create(PreprocessingType.Default)
        )

        tokenizer = TransformerTokenizer(
            self.model_name, create_encoder_from_path(path_authors)
        )
        ...
        for trainable in [True, False]:
            ...
            experiment_id = (
                "Trainable"
                + os.path.sep
                + f"Train_{trainable}_stamp:{str(current_timestamp)}"
            )
            print(experiment_id)
            ...
            summarization.inspect_set(
                train, ExperimentSummarizationFields.TrainRecords.value
            )
            summarization.inspect_set(
                valid, ExperimentSummarizationFields.ValidRecords.value
            )
            summarization.inspect_set(
                test, ExperimentSummarizationFields.TestRecords.value
            )
            ...
            wrapper.run(
                ...
                .prefetch(4),
                prepare_dataset_from_tokenizer(valid, tokenizer).batch(1),
                prepare_dataset_from_tokenizer(test, tokenizer).batch(1),
                settings,
                description,
                False,
            )
